...
2. **Run the six-stage pipeline from `rules.md`**, writing each output as its own file in
   ...
   - `06_preview/preview.html` (the headline deliverable: a self-contained, brand-styled HTML **brochure for the demo** — visual and skimmable, not a memo; framed around the customer's outcome so they get excited to attend the demo. Athletic-retail editorial theme: ink-black (#111) hero/CTA bands alternating with white bands, stitched by a gradient ribbon (pink→purple→teal) in the hero; sentence-case display sans, section eyebrows in small uppercase sans (no "//"), mono kept only for small card tags / table header; 4px-rounded hairline cards; soft pink/purple tints on stat tiles with dark ink text, sale-red / success-green for signals in the readout, featured card flips to solid black, white text only on black. Build on `reference/output-templates/06_preview.html`, which already encodes the theme)
