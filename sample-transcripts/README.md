...
These are fresh, unprocessed transcripts you can drop into DemoWorks to demonstrate the
...
- **`harbor-point-appeals-intake.md`** — A regional Medicaid managed-care plan whose loudest
  ...
  *Pattern 1 (public-facing inquiry intake & triage)*, with Caseboard (Pattern 3) supporting.
  Good for showing that DemoWorks exercises judgment rather than repeating one answer.

## How to run it in a video (≈ the on-camera flow)

DemoWorks is set up so that **pasting a transcript is the only action needed** — the root
`CLAUDE.md` tells Claude Code to run the pipeline automatically when it receives a transcript.

1. **Show the brief.** Open `brief.md` — the real problem DemoWorks solves.
...
> can show DemoWorks reasoning to *Pattern 1 (intake & triage)*, a different lead than Summit
> Ridge's Pattern 6.
